...
def save_to_csv(sensor_id, data):
    ...
    """
    # Get the district from the data, default to 'Unknown_District' if not found
    district = data.get('district', 'Unknown_District')
    
    # Create a Logger instance for the given district and sensor
    logger = Logger(district, sensor_id)
    ...
    try:
        ...
        with open(filename, 'a', newline='') as file:
            ...
            logger.setup_logger().info(f"Data saved to CSV: {filename}")

    except Exception as e:
        # Log any error that occurs during CSV writing
        logger.setup_logger().error(f"Error saving data to CSV for {sensor_id} in {district}: {e}")
